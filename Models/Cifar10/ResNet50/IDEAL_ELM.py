...
# --Repeat the following steps until there are samples--
    ...
    for i in range(1,numPts):
        ...
        for j in range(0,clustCent.shape[0]):
            ...
            test1 = distance[0,j]**2 
            test_2 = radius**2+radius**2
            if (distance[0,j]**2 < 2*radius).any():
        ...
        # Consider X belongs to nearest cluster centers and update mean
        if Index.size != 0:
            neighbourAviable = 1
...
class IDEALClassifier:
    ...
    def PrototypesIdentification(self, Image,GlobalFeature,LABEL,CL):
        ...
        for i in range(0, CL+1):
            ...
            data[i] = data[i] / (Xnorm.reshape(-1,1))*(np.ones((1,W)))
            # bandwidth = estimate_bandwidth(data[i], quantile=0.3)
            # print(f'bandwidth: {bandwidth}')
            Prototypes[i] ,data2cluster,variance = ELMean(data[i],0.25)
...
if __name__ == '__main__':
    ...
    for each_shuffle in range(1):
        ...
        model.save_model(x,r'/mmfs1/scratch/hpc/00/zhangz65/Code/IDEAL/code/update_on-cifar10/models/ELM_resnet50_'+str(each_shuffle)+'_model')
