...
class IDEALClassifier:
    ...
    def IDEALclassifier(self, Data,Image):
        ...
        Data_2 = Data**2
        Data_2 = Data_2.reshape(-1, 2048)
        Xnorm = np.sqrt(np.sum(Data_2,axis=1))
        
        data = Data.reshape(-1,2048) / (Xnorm.reshape(-1,1))*(np.ones((1,W)))
        Centre = data[0,]
        Centre = Centre.reshape(-1,2048)
...
if __name__ == '__main__':
    ...
    for each_shuffle in range(5):
        ...
        model.save_model(x,r'/mmfs1/scratch/hpc/00/zhangz65/Code/IDEAL/code/cifa10/training_on_cifa10/model/IDEAL_resnet101_'+str(each_shuffle)+'_model')
