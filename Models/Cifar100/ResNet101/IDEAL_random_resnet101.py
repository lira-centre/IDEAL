...
class IDEALClassifier:
    ...
    def PrototypesIdentification(self, Image,GlobalFeature,LABEL,CL,random_state):
        ...
        for i in range(0, CL+1):
            ...
            print(data[i].shape)
            Prototypes[i] = data[i][np.random.choice(data[i].shape[0], 250, replace=False)]
...
if __name__ == '__main__':
    ...
    for each_shuffle in range(5):
        ...
        model.save_model(x,r'/mmfs1/scratch/hpc/00/zhangz65/Code/IDEAL/code/update_on_cifar100/models/random_resnet101_'+str(each_shuffle)+'_model')
